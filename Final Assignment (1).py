...
table_bs=soup.find_all('table')
for i,row in enumerate(table_bs):
    if ('Tesla Quarterly Revenue' in str(row)):
        index=i
tesla_revenue=pd.DataFrame(columns=['Date','Revenue'])
for j in table_bs[index].tbody.find_all('tr'):
    col=j.find_all('td')
    date=col[0].text
    revenue=col[1].text.replace('$','').replace(',','')
    df=pd.DataFrame({'Date':[date],'Revenue':[revenue]})
    tesla_revenue=tesla_revenue.append(df,ignore_index=True)
tesla_revenue
...
for i in table_bs[index].tbody.find_all('tr'):
    col=i.find_all('td')
    if col!=[]:
        Date=col[0].text
        Revenue=col[0].text.replace('$','').replace(',','')
